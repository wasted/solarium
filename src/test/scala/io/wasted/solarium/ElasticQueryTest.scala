...
class ElasticQueryTest extends JUnitSuite {
  ...
  def hoboPrepIndex() {
    ...
    try {
      ...
      case _: Throwable => println("Error creating geopanda stuff, may allready exist")
    }
    geoClient.admin().indices().prepareRefresh().execute().actionGet()
  ...
  def hoboDone() {
    ...
    try {
      ...
      case _: Throwable => println("Error cleaning up after tests... oh well")
    }
  }
