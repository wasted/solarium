...
object Ast {
  ...
  case class Field(fieldName: String) extends ScoreBoost {
    ...
    def elasticBoost(): (List[String], String) = {
      Tuple2(Nil, "doc['" + fieldName + "'].value")
    }
  }
  ...
  case class WeightedField(fieldName: String, weight: Double = 1) extends ScoreBoost {
    ...
    def elasticBoost(): (List[String], String) = {
      weight match {
        case 1.0 => Tuple2(Nil, "doc['" + fieldName + "'].value")
        case _ => Tuple2(Nil, "(doc['" + fieldName + "'].value *" + weight.toString + ")")
      }
    }
  }
  ...
  case class GeoDist(name: String, lat: Double, lng: Double, distType: String = "") extends ScoreBoost {
    ...
    /** @inheritdoc */
    def elasticBoost(): (List[String], String) = {
      val distanceInKm = Tuple2(List(lat, lng).map(_.toString), "doc['" + name + "'].distanceInKm(%s,%s)")
      distType match {
        case "square" => Tuple2(distanceInKm._1, "pow(%s,2.0)".format(distanceInKm._2))
        case _ => distanceInKm
      }
    }
  }
  ...
  case class Phrase[T](query: T, escapeQuery: Boolean = true) extends Query[T] {
    ...
    /** @inheritdoc */
    def elasticExtend(qf: List[WeightedField], pf: List[PhraseWeightedField], mm: Option[String]): ElasticQueryBuilder = {
      val q = EQueryBuilders.queryStringQuery(this.extend())
  ...
  case class BagOfWords[T](query: T, escapeQuery: Boolean = true) extends Query[T] {
     ...
     */
    def elasticExtend(qf: List[WeightedField], pf: List[PhraseWeightedField], mm: Option[String]): ElasticQueryBuilder = {
      val normalq = EQueryBuilders.queryStringQuery(this.extend())
  ...
  case class Boost[T](q: Query[T], weight: Float) extends Query[T] {
    ...
    /** @inheritdoc */
    def elasticExtend(qf: List[WeightedField], pf: List[PhraseWeightedField], mm: Option[String]): ElasticQueryBuilder = {
      EQueryBuilders.functionScoreQuery(q.elasticExtend(qf, pf, mm)).boost(weight)
    }
  }
}
